		...
		if (editor.addFeature)
		{
			editor.addFeature( {
				...
				allowedContent: {
					'table td th': {
						propertiesOnly: true,
						attributes: 'background,padding',
						styles: 'background-repeat,background-position,background-size,padding'
					}
				}
			} );
		}
...
CKEDITOR.on( 'dialogDefinition', function( ev )
	{
		...
		var padding =  {
				...
				id : 'padding',
                setup: setupCells(function (selectedElement) {
                    return selectedElement.getStyle('padding');
                }),
                onChange: function() {
                    var stylesInput = this.getDialog().getContentElement('advanced', 'advStyles');

                    if (stylesInput) {
                        stylesInput.updateStyle('padding', this.getValue());
                    }
                },
                commit: function (data, selectedElement) {
                    var element = selectedElement || data,
                        value = this.getValue(),
                        oBackground = this.getDialog().getContentElement(tabName, "padding"),
                        background = oBackground && oBackground.getValue();

                    if (value && background)
                        element.setStyle('padding', value);
                    else
                        element.removeStyle('padding');
                }
			};
		...
		var backgroundSize = {
			...
			commit: function (data, selectedElement) {
				...
				if (value && background)
					...
					element.removeStyle('background-size');
			}
		};
		...
		if (tabName == 'advanced')
		{
			...
			tab.add({
				type: 'hbox',
				widths: ['', '100px'],
				children: [backgroundPosition, backgroundSize, padding, backgroundRepeat]
			});
			tab.add({
				type: 'hbox',
				widths: ['50%', '50%'],
				//children: [backgroundAttachment, backgroundColor]
                children: [backgroundAttachment]
			});
		}
		else
		{
...
// Translations
	...
	addTranslation('pt-br', {
		...
		repeatY : 'Vertical',
		repeatNone : 'Nenhum',
		left_top : 'Esquerda Superior',
		left_center :'Esquerda Centro',
		left_bottom : 'Esquerda Inferior',
		center_top : 'Centro Superior',
		center_center : 'Centro',
		center_bottom :'Centro Inferior',
		right_top : 'Direita Superior',
		right_center :' Direita Centro',
		right_bottom : 'Direita Inferior'
	});
